...
def apoe_to_std(x):
    if pd.isna(x):
        return np.nan
    # keep your original numeric handling
...
def harmonize_obs_from_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # APOE
    # Robust APOE column detection (case-insensitive, ignores spaces/punctuation)
    apoe_cols = [
        c for c in df.columns
        if re.search(r"apoe\s*[_\- ]*\s*genotype", c, flags=re.I)
    ]
    if not apoe_cols:
        # fall back to any column containing APOE
        apoe_cols = [c for c in df.columns if "apoe" in c.lower()]

    if apoe_cols:
        apoe_src = df[apoe_cols[0]]
    else:
        apoe_src = pd.Series(np.nan, index=df.index)
