...
# ---------- COMM & MEMORY SAFETY (drop-in) ----------
import os

# Keep your existing RMM/Dask memory pool settings:
...
# Force Dask to use TCP (disable UCX completely).
# This avoids UCXX endpoint timeouts you’re seeing under heavy load in screen.
...
os.environ["DASK_DISTRIBUTED__COMM__RETRY__COUNT"] = "5"

# Give comms more time before deciding a peer is dead.
os.environ["DASK_DISTRIBUTED__COMM__TIMEOUTS__CONNECT"] = "60s"
os.environ["DASK_DISTRIBUTED__COMM__TIMEOUTS__TCP"] = "60s"

# Optional (reduces cross-worker movement → fewer comms)
os.environ["DASK_DISTRIBUTED__SCHEDULER__WORK_STEALING"] = "false"

# If you ever want to flip back to UCX, change this to True
USE_UCX = False
...
DONOR_KEY        = "Donor ID"
ROW_CHUNK        = 4000
TEST_MAX_CELLS   = None      # <-- run all cells
SCRUBLET_PCS     = 15
SCRUBLET_SIMR    = 2.0
SCRUBLET_RATE    = 0.045
PER_DONOR_MAX    = 12000
...
def start_cluster():
    ...
    try:
        ...
        # hard fallback to TCP if UCX fails during construction
        cluster = LocalCUDACluster(
            n_workers=N_WORKERS,
            threads_per_worker=1,
            protocol="tcp",
