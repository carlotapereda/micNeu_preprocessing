...
Handles >4M cells
Usage:
    python plot_umap_from_merged_v8.py \
...
#########################
def run_pipeline(adata_hvg, n_pcs=20, n_neighbors=10, resolution=0.5):
    ...
    ########## GPU UMAP (FAST + uses RAPIDS neighbors) ##########
    log("Running UMAP on GPU with RAPIDS…")
    rsc.tl.umap(
        adata_hvg,
        min_dist=0.5,
        spread=1.0,
    ...
    log("✓ GPU UMAP complete.")

    ########## NOW convert everything back to CPU ##########
    log("Converting matrices + graphs + UMAP to CPU…")
    ...
    # UMAP embedding
    if "cupy" in type(adata_hvg.obsm["X_umap"]).__module__:
        adata_hvg.obsm["X_umap"] = cp.asnumpy(adata_hvg.obsm["X_umap"])
