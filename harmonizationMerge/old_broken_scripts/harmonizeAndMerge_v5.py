...
def normalize_sex_values(series: pd.Series) -> pd.Categorical:
    ...
    s = s.astype("string").str.strip().str.lower().replace({
...
def derive_race_from_seaad(df: pd.DataFrame) -> pd.Series:
    ...
    if not present or "dataset" not in df.columns:
        log("derive_race_from_seaad: no checkbox columns or dataset missing"); return out
    mask = df["dataset"].astype(str).eq("SEAAD")
    if not mask.any():
        log("derive_race_from_seaad: no SEAAD rows"); return out
    def _one(row):
    ...
    log(f"derive_race_from_seaad: top labels {vc}"); return out
...
def build_obs_union_from_sources(files_map, merged_index, merged_dataset, index_unique="-"):
    ...
    for ds_key, path in files_map.items():
        ...
        n_sel = int(sel.sum())
        if not n_sel:
            log(f"{ds_key}: no rows in merged; skip"); continue
        ...
        # discover dynamic columns we might need (sex variants; alias names)
        ...
        keep = sorted(set(keep + dynamic_sex + dynamic_braak + dynamic_cerad))
...
#  (CellTypist columns are NOT altered; we will pass them through unchanged below.)
# ==========
def harmonize_obs_from_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Sex (dataset-specific → normalize)
...
def main():
    ...
    passthrough_cols = [
        ...
        or c in ["Class","Subclass","Supertype","Class confidence","Subclass confidence","Supertype confidence"]
    ]
    ...
    log(f"Non-null braak_stage={obs.get('braak_stage', pd.Series(index=obs.index)).notna().sum():,}, "
    ...
    try:
        if hasattr(m, "file") and hasattr(m.file, "close"):
            m.file.close(); log("Closed backed handle")
    except Exception as e:
