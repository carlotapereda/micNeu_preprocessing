...
print("✅ Preprocessing complete on GPU")

########################################
# PCA / UMAP
########################################

rsc.pp.pca(adata, n_comps=50, use_highly_variable=False) # With use_highly_variable = False we save VRAM since we already subset the matrix to only HVGs.
sc.pl.pca_variance_ratio(adata, log=True, n_pcs=50)

rsc.get.anndata_to_CPU(adata) #Now we move .X and .layers out of the GPU.
...
pred = annotate(
    adata_celltypist,
    model=model,
    use_gpu=True,          # ✅ new GPU mode (RAPIDS/cuML backend)
    batch_size=200_000, #INCREASE TO 400,0000 OR 500,000 ONCE IT RUNS 
...
# Optionally store raw counts as well (redundant since X is counts)
