import anndata as ad
import pandas as pd
import numpy as np

############################################
# Source
############################################

SRC = "../../celltypist/mit_celltypist_GPU_counts_only.h5ad"

print("📖 Reading MIT-ROSMAP AnnData (backed)...")
adata = ad.read_h5ad(SRC, backed="r")

############################################
# Copy obs (DO NOT edit adata.obs)
############################################

obs = adata.obs.copy()

############################################
# [MIT-ROSMAP] 1. Remove any columns that are not these
############################################

KEEP_COLS = [
    # QC / counts
    "n_genes_by_counts","log1p_n_genes_by_counts","total_counts","log1p_total_counts",
    "pct_counts_in_top_20_genes","total_counts_mt","log1p_total_counts_mt","pct_counts_mt",
    "total_counts_ribo","log1p_total_counts_ribo","pct_counts_ribo",
    "total_counts_hb","log1p_total_counts_hb","pct_counts_hb",
    "n_genes","doublet_scores","predicted_doublets","doublet_label","outlier","mt_outlier",

    # Metadata
    "individualID","projid","msex","educ","race","apoe_genotype","age_death",
    "cts_mmse30_lv","pmi","braaksc","ceradsc","cogdx",
    "celltypist_cell_label","Study","celltypist_conf_score","batch",
]

obs = obs[[c for c in KEEP_COLS if c in obs.columns]]

############################################
# [MIT-ROSMAP] 2. Create new column called APOE_Genotype
# keep apoe_genotype column
############################################

APOE_MAP = {
    33: "3/3",
    34: "3/4",
    44: "4/4",
}

obs["APOE_Genotype"] = obs["apoe_genotype"].map(APOE_MAP)

############################################
# [MIT-ROSMAP] 3. Create new column called Cognitive_Status
# keep cogdx column
############################################

def map_cognitive_status(val):
    if val in [1, 2, 3]:
        return "No dementia"
    if val in [4, 5, 6]:
        return "Dementia"
    return np.nan

obs["Cognitive_Status"] = obs["cogdx"].apply(map_cognitive_status)

############################################
# [MIT-ROSMAP] 4. Create new column called Sex
# keep msex column
############################################

SEX_MAP = {
    1: "Male",
    0: "Female",
}

obs["Sex"] = obs["msex"].map(SEX_MAP)

############################################
# [MIT-ROSMAP] 5. Create new column called Age_Death
# keep age_death column
############################################

def normalize_age(val):
    if isinstance(val, str) and val.strip() == "90+":
        return 90
    return val

obs["Age_Death"] = obs["age_death"].apply(normalize_age)

############################################
# [MIT-ROSMAP] 6. Create new column called Race
# keep race column
############################################

RACE_MAP = {
    1: "White",
    2: "Black and African American",
    3: "American Indian or Alaska Native",
    4: "Native Hawaiian or Other Pacific Islander",
    5: "Asian",
    6: "Other",
    7: "Unknown",
}

obs["Race"] = obs["race"].map(RACE_MAP)

############################################
# [MIT-ROSMAP] 7. Create Dataset column
############################################

obs["Dataset"] = "MIT-ROSMAP"

############################################
# [MIT-ROSMAP] 8. Create barcode column
############################################
# make vars unique, ensure barcodes are unique
# obs["raw_barcode"] = obs.index
# concatenate Dataset + index

obs.index = obs.index.astype(str)
obs["raw_barcode"] = obs.index
obs["barcode"] = obs["Dataset"] + "_" + obs["raw_barcode"]

############################################
# [MIT-ROSMAP] 9. Create column called chemistry
############################################

obs["chemistry"] = "10x 3’ v3"

############################################
# [MIT-ROSMAP] 10. Copy columns under new names (keep both)
############################################

COPY_MAP = {
    "individualID": "Donor_ID",
    "educ": "Years_Education",
    "cts_mmse30_lv": "Last_MMSE",
    "braaksc": "Braak",
    "ceradsc": "CERAD",
}

for src, dst in COPY_MAP.items():
    if src in obs.columns:
        obs[dst] = obs[src]

############################################
# [MIT-ROSMAP] 11. Save
############################################
# Drop CSV artifacts
obs = obs.drop(columns=["Unnamed: 0", "barcode.1"], errors="ignore")

print("💾 Saving harmonized MIT-ROSMAP obs...")
obs.to_csv("MIT_ROSMAP_harmonized_obs.csv", index=False)

############################################
# Print obs names + summaries
############################################

summary_rows = []

for col in obs.columns:
    if pd.api.types.is_numeric_dtype(obs[col]):
        desc = obs[col].describe()
        for stat, val in desc.items():
            summary_rows.append(
                {"column": col, "stat": stat, "value": val}
            )
    else:
        vc = obs[col].value_counts(dropna=False)
        for val, cnt in vc.items():
            summary_rows.append(
                {"column": col, "stat": val, "value": cnt}
            )

summary_df = pd.DataFrame(summary_rows)
summary_df.to_csv("MIT_ROSMAP_obs_column_summary.csv", index=False)

print("✅ MIT-ROSMAP harmonization complete.")
print(f"Columns ({len(obs.columns)}):")
print(list(obs.columns))
